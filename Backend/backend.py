...
class Backend:
  ...
  def __init__(self):
    ...
    argparser.add_argument("-us", "--user", help="username for MQTT broker", required=True)
    argparser.add_argument("-pw", "--password", help="password for MQTT broker", required=True)
  ...
  def on_mqtt_message(self, client, config, msg):
    ...
    if node_id == self.config.node:
      ...
      # look for returned file data which we can parse, in this example file 1
      my_sensor_value = 0

      for action in cmd.actions:
        if type(action.operation) is ReturnFileData and action.operand.offset.id == 1:
          my_sensor_value = struct.unpack("cccc", bytearray(action.operand.data))[teller]  # parse binary payload (adapt to your needs)
